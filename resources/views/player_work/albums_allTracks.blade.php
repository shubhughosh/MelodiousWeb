    ...
    <div class="container p-4">
        <div class="row">
            <div class="col-lg-10 mx-auto">
                <div class="card border-0 shadow-sm" style="background-color: rgba(206, 109, 109, 0.26)">
                    <div class="card-body">
                        @foreach ($album_cover as $abmc)
                        <div class="row">
                            <div class="col-lg-3">
                                <img src="{{asset('album_cover/'.$abmc->image)}}" class="card-img-top">
                            </div>
                            <div class="col-lg-6 mt-1">
                                <h4>Best of {{$abmc->album_name}}</h4>
                                <small>Language : {{$abmc->album_language_type}} </small><br>
                                @endforeach
                                <small>Created by MelodiousWeb | Tracks {{$alltracks->count()}}</small>
                            </div>
                        </div>
                        <table class="table table-borderless text-white mt-4">
                            <tr>
                                <th>#id</th>
                                <th>Title</th>
                                <th>Artist</th>
                            ...
                            @foreach ($alltracks as $alltrk)
                            <tr style="border-bottom: 0.1rem solid rgb(228, 227, 227);margin-bottom:2px">
                                <td>{{$alltrk->id}}</td>
                                <td> <i class="fas fa-music text-danger me-2"></i> {{$alltrk->name}}</td>
                                <td>{{$alltrk->singer}}</td>
                                <td>{{$alltrk->genres}}</td>
                                <td>
                                    <a href="{{ route('albums',['id'=>$alltrk->id,'name'=>$alltrk->name,'album_name'=>$alltrk->album_name]) }}" class="btn btn-danger btn-sm" style="border-radius:100px"><i class="fas fa-play"></i></a>
                                </td>
                            </tr>
                            @endforeach
                        </table>
